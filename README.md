...
결과가 너무 길어 한 번에 못 붙여넣는 경우(특히 이미지가 든 파일)를 위한 옵션입니다.
`조각내기` 칸에서 규칙을 고릅니다.
...
기준은 **원본 크기가 아니라 결과 텍스트의 글자수**입니다. 압축이 얼마나 먹힐지는
해 보기 전에 알 수 없으므로, 일단 다 만든 다음 길이를 재서 판단합니다.
그래서 "3 MB짜리 문서"가 나뉠지 아닐지는 내용에 따라 달라집니다 —
텍스트가 많으면 잘 줄어 한 덩어리로 남고, 이미지가 많으면 나뉩니다.
실행 뒤 결과줄에 `한도를 넘어 자동으로 나눔` / `한도 안이라 나누지 않음`으로 어느 쪽이었는지 알려줍니다.

기본값 20만은 **붙여넣는 쪽이 지연 없이 받아주는 한도를 실측해** 잡은 값입니다.
붙여넣을 곳의 한도가 다르면 `tools\make-netcus-length-tests.ps1`로 그 크기의
실제 블록을 만들어 직접 재 보고 프리셋을 바꾸면 됩니다.

실제 파일로 재 본 조각 수(20만 자 기준):

| 원본 | 결과 글자수 | 조각 |
|---|---:|---:|
| 엑셀 4 KB | 5,610 | 1 |
| 소스코드 25개 971 KB (아카이브, 압축률 32%) | 307,358 | 2 |
| PDF 190 KB | 252,302 | 2 |
| **이미지가 잔뜩 든 PDF 15.9 MB** | 19,165,094 | **96** |

문서·소스·엑셀은 1~2조각이라 문제가 없습니다. 조각이 폭발하는 건 이미지가
잔뜩 든 대용량 파일뿐이고, 대략 **압축 안 되는 원본 147 KB당 1조각**(1 MB당 7조각)입니다.
그런 파일은 한도를 키우기보다 **넣기 전에 원본에서 이미지를 줄이는 쪽**이 훨씬
효과가 큽니다(보통 1/5~1/10).
...
| 이미지·동영상·ZIP·EXE 등 큰 바이너리 | 압축이 안 돼 **원본보다 커짐** (136%). 기본 설정이면 한도를 넘는 순간 알아서 나뉩니다 |
