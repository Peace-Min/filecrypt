...
#
# Makes real, decryptable FileCrypt blocks of exactly the requested character
# counts. Paste each one into the target field to find the largest size that
# fits, then copy it back out and decrypt it to confirm the round trip was
# lossless. The biggest size that both fits AND decrypts is your split size.
#
...
Write-Host ''
Write-Host ('{0,-11} {1,-13} {2,-7} {3,-32} {4}' -f 'target', 'actual', 'diff', 'file', 'decode')
Write-Host ('-' * 80)
...
foreach ($N in $Targets) {
    ...
    $f = Join-Path $OutDir ('netcus-length-{0}.txt' -f $N)
    ...
    Write-Host ('{0,-11:N0} {1,-13:N0} {2,-7} {3,-32} {4}' -f `
